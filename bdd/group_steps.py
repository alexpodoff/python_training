...
@given('a modify info with <name>, <header> and <footer>')
def mod_group(name, header, footer):
    return Group(name=name, header=header, footer=footer)


@when('I modify the group from the list')
def modify_contact(app, random_group, mod_group):
...
def verify_group_modified(db, non_empty_group_list, random_group, mod_group, app, check_ui):
    old_groups = non_empty_group_list
    ...
    if check_ui:
        assert sorted(
            new_groups, key=Group.id_or_max) == sorted(app.group.get_contact_list(), key=Group.id_or_max)